...
public struct CommandCodeUsageSnapshot: Sendable {
    ...
    private func makeMonthlyWindow() -> RateWindow? {
        guard let total = self.monthlyCreditsTotal, total > 0 else {
            // Without a reported grant, its size comes from the optional subscription lookup. When that
            // lookup times out or fails, the plan is unknown rather than absent: reporting the free-tier
            // reading below would show an untouched monthly bar for a paid plan that is partly or fully spent.
