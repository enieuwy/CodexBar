    ...
    async function webQuota() {
      ...
      try {
        ...
        for (const team of teams.slice(0, 2)) {
          const id = (team as Record<string, unknown> | null)?.team_id;
          const teamID = typeof id === "string" ? id : Number.isSafeInteger(id) ? String(id) : "";
          if (!/^[0-9]+$/.test(teamID)) continue;
          const quota = (await get(`/api/portal/teams/${teamID}/subscription-quota`))?.subscription_quota;
          const parsed = quota && typeof quota === "object" ? parseWebQuota(quota as Record<string, unknown>) : null;
