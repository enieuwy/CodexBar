...
- Keychain: check a credential item's partition list before no-UI reads, so background checks of Claude Code, Muse Code, and other CLI-owned logins whose owner rewrote the partitions report that access needs approval instead of showing a password prompt. Claude's "Always" Keychain prompt setting can still prompt in the background by design (refs #3798).
